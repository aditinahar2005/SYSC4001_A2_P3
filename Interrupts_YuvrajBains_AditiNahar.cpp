 ...
 *
 * @file interrupts.cpp
 * @author Sasisekhar Govind, Yuvraj Bains, Aditi Nahar
 *
 */

#include "interrupts_YuvrajBains_AditiNahar.hpp"
    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        if(activity == "CPU") { //As per Assignment 1
            ...
            if(!allocate_memory(&child)) {
                std::cerr << "ERROR! Memory allocation failed during FORK!" << std::endl;
            }
...

            {
                ...
                hdr << "time: " << current_time << "; current trace: FORK, " << duration_intr << "\n";
                system_status += hdr.str();
            ...
            {
                auto [child_exec, child_sys, child_time] =
            ...
            if(!wait_queue.empty()) {
                ...
                if (terminated_child.partition_number != -1) { //free
                    free_memory(&terminated_child);
                }
            }
            ///////////////////////////////////////////////////////////////////////////////////////////
            ...
            exec_counter++;
            int exec_intr_num = (exec_counter == 1 ? 3 : 4);

            auto [intr, time] = intr_boilerplate(current_time, exec_intr_num, 10, vectors);
            execution += intr;
            current_time = time;
            ...
            unsigned int prog_mb = get_size(program_name, external_files);
            int load_time = static_cast<int>(prog_mb) * 15;
            ...
            if(!allocate_memory(&current)) {
                std::cerr << "ERROR! Memory allocation failed during EXEC!" << std::endl;
            }
            ...
            {
                std::ostringstream hdr;
                hdr << "time: " << current_time << "; current trace: EXEC " << program_name << ", " << duration_intr << "\n";
                system_status += hdr.str();
                system_status += print_PCB(current, wait_queue);
                system_status += "\n";
            }
            ...
            {
                auto [exec_exec, exec_sys, exec_time] =
